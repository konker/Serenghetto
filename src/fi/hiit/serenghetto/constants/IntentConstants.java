package fi.hiit.serenghetto.constants;

public class IntentConstants {
    public static final String NEW_BEST_LOCATION_ESTIMATE_INTENT = "fi.hiit.serenghetto.NEW_BEST_LOCATION_ESTIMATE";
    public static final String NEW_BEST_LOCATION_ESTIMATE_EXTRA_LOCATION = "NEW_BEST_LOCATION_ESTIMATE_EXTRA_LOCATION";

    public static final String NEW_BARCODES_INTENT = "fi.hiit.serenghetto.NEW_BARCODES";
    public static final String NEW_BARCODES_EXTRA_COUNT = "NEW_BARCODES_EXTRA_COUNT";
}
